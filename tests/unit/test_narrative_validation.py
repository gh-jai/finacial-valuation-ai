...
def inputs() -> tuple[dict, dict, set[str], set[str]]:
    document = json.loads(
        (ROOT / "benchmarks/fixtures/narratives/synthetic-base.json").read_text(
            encoding="utf-8"
        )
    )
    schema = json.loads(
    ...
    catalog = yaml.safe_load(
        (ROOT / "sources/catalog.yaml").read_text(encoding="utf-8")
    )
...
def test_three_p_hierarchy_rejects_higher_level_pass() -> None:
    document, _, _, _ = inputs()
    document["three_p"]["possible"]["result"] = "fail"
    document["three_p"]["plausible"]["result"] = "pass"
    document["three_p"]["probable"]["result"] = "pass"
    assert any("plausible cannot pass" in error for error in errors_for(document))
    assert any("probable cannot pass" in error for error in errors_for(document))


def test_three_p_overall_matches_highest_consecutive_pass() -> None:
    document, _, _, _ = inputs()
    document["three_p"]["probable"]["result"] = "uncertain"
    document["three_p"]["overall"] = "probable"
    assert any("highest consecutive passed level: plausible" in error for error in errors_for(document))
...
def test_missing_assertion_mapping_is_rejected() -> None:
    document, _, _, _ = inputs()
    document["value_driver_mappings"] = [
        item
        for item in document["value_driver_mappings"]
        if item["assertion_id"] != "NAR-A-001"
    ]
...
def test_feedback_history_cannot_erase_prior_version() -> None:
    revised = json.loads(
        (ROOT / "benchmarks/fixtures/narratives/synthetic-revised.json").read_text(
            encoding="utf-8"
        )
    )
