...
def validate_narrative_document(
    ...
    for assertion in assertions:
        ...
        if status == "unquantified-limitation" and "limitation" not in assertion.get("statement", "").lower():
            errors.append(f"unquantified assertion {assertion_id} must explicitly state its limitation")
    ...
    for level in ("possible", "plausible", "probable"):
        assessment = document.get("three_p", {}).get(level, {})
...
def validate_repository(root: Path = ROOT) -> tuple[list[str], int]:
    ...
    for path in paths:
        ...
        errors.extend(f"{path.relative_to(root)}: {error}" for error in validate_narrative_document(document, schema, source_ids, claim_ids))
    document_ids = {item.get("id") for item in documents}
    for document in documents:
        for alternative in document.get("alternatives", []):
            if alternative["status"] == "active" and alternative["narrative_id"] not in document_ids:
                errors.append(f"{document['id']}: active alternative {alternative['narrative_id']} has no separate document")
